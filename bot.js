...
async function selectWallet(wallets) {
  ...
  return new Promise((resolve) => {
    rl.question("\nPilih nomor wallet (contoh: FIRST atau 1): ", (answer) => {
      rl.close();
...
async function sendToken(wallet, to, valueEther, retries = 3, delayMs = 3000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const isRetryable = err.code === "ECONNRESET" || err.code === "ETIMEDOUT" || err.message?.includes("socket hang up");
      if (isRetryable && attempt < retries) {
        console.log(`\n⚠️  Koneksi gagal (${err.code}), retry ${attempt}/${retries - 1}...`);
